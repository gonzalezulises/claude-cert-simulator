...
export const examQuestionsEs: Record<string, QuestionTranslation> = {
  ...
  "E3-011": {
    ...
    options: {
      A: "`model: claude-sonnet-5` — especificar un modelo hace que el skill se ejecute en una llamada API separada",
  ...
  "E4-001": {
    question:
      "Tu aplicación fue construida sobre Claude 3.5 y dependía de respuestas asistente pre-completadas (comenzando el turno del asistente con JSON parcial) para forzar salida estructurada. Actualizas a Claude 5. ¿Qué debes cambiar en tu integración?",
    options: {
      A: "Nada — las respuestas pre-completadas están completamente soportadas en Claude 5 y siguen siendo el enfoque recomendado",
      B: "Las respuestas pre-completadas están deprecadas en Claude 5 — reemplázalas con la característica de salida estructurada `output_config.format` o instrucciones explícitas en el prompt",
  ...
  "E4-010": {
    question:
      "Un equipo migra su prompt de una versión anterior de Claude a Claude 5. Una herramienta que se llamaba ~60% de las veces ahora se activa en casi cada mensaje, causando efectos secundarios inesperados. ¿Cuál es la causa más probable?",
    options: {
      A: "Claude 5 tiene un error en la selección de herramientas que fuerza a que todas se activen",
      B: "El prompt estaba sobre-diseñado para un modelo más antiguo y menos capaz — las instrucciones explícitas de uso de herramientas que eran necesarias para activar la herramienta ahora causan sobreactivación con el seguimiento de instrucciones más capaz de Claude 5",
  ...
  "E5-001": {
    question:
      "Un equipo está eligiendo modelo para una tarea de análisis de documentos largos. Cada documento tiene 800 000 tokens. ¿Qué modelos pueden procesar un documento de ese tamaño de forma nativa, sin fragmentarlo ni configurar nada especial?",
    options: {
      A: "Solo Claude Opus 5 — cualquier otro modelo actual se queda en 200K tokens",
      B: "Tanto Claude Opus 5 como Claude Sonnet 5 tienen ventanas de contexto de 1M tokens y admiten el documento tal cual; Claude Haiku 4.5 se queda en 200K y obligaría a fragmentarlo",
      C: "Ningún modelo actual admite 800K tokens; hay que fragmentar los documentos por debajo de 200K sea cual sea el modelo",
      D: "Todos los modelos Claude actuales tienen ventanas de 2M tokens, así que sirve cualquiera",
    },
    explanation:
      "Las ventanas de contexto cambian según el modelo, y esa diferencia es justo lo que decide la elección. Claude Opus 5 y Claude Sonnet 5 tienen una ventana de 1M tokens — en Opus 5 es a la vez la predeterminada y el máximo—, así que un documento de 800K no necesita configuración alguna. Claude Haiku 4.5 tiene 200K, de modo que el mismo documento habría que trocearlo y recomponerlo después. La opción A se equivoca porque Sonnet 5 también llega a 1M. La C se equivoca porque 800K cabe de sobra en 1M. La D exagera: 2M no es una ventana que Claude ofrezca.\n\nLa lectura arquitectónica: elige el modelo por el tamaño de la entrada, no solo por la dificultad de la tarea. Bajar a un modelo más barato con una ventana menor te compra una tubería de fragmentación que luego hay que mantener.\n\nCómo implementarlo:\n```python\nimport anthropic\n\nclient = anthropic.Anthropic()\n\n# El contexto de 1M es nativo: el documento de 800K entra entero.\nresponse = client.messages.create(\n    model=\"claude-sonnet-5\",\n    max_tokens=4096,\n    messages=[{\"role\": \"user\", \"content\": documento_800k}],\n)\n\nprint(\"Tokens de entrada usados:\", response.usage.input_tokens)\n```\n\n```python\n# No adivines la ventana: pregúntasela a la Models API antes de casarte con un modelo.\nm = client.models.retrieve(\"claude-sonnet-5\")\nprint(m.max_input_tokens)   # ventana de contexto\nprint(m.max_tokens)         # tope de salida, que es otro límite\n```",
    keyConcept: "Ventanas de contexto: Opus 5 y Sonnet 5 = 1M nativo; Haiku 4.5 = 200K",
  },
  ...
  "E5-010": {
    question:
      "Un equipo de soporte procesa tickets con Claude Haiku 4.5 y se topa con un hilo de 180 000 tokens que supera la ventana de contexto de 200K del modelo en cuanto se suman el prompt de sistema y las definiciones de herramientas. Necesitan el hilo completo en contexto. ¿Cuál es el enfoque técnico correcto?",
    options: {
      A: "Mover esta carga a Claude Sonnet 5 o Claude Opus 5, que tienen ventanas de 1M tokens de forma nativa",
      B: "Añadir un header beta de contexto extendido para desbloquear 1M tokens en Claude Haiku 4.5",
      C: "Poner `extended_context: true` en la configuración de servidores MCP de `.mcp.json`",
      D: "Subir `max_tokens` a 1000000 para que la solicitud acepte la entrada más grande",
    },
    explanation:
      "Solo la opción A describe un mecanismo que existe. Claude Sonnet 5 y Claude Opus 5 tienen ventanas de 1M tokens de forma nativa, así que el hilo cabe sin header, sin bandera y sin fragmentar: cambias de modelo y el problema desaparece.\n\nLas otras tres son los errores que conviene saber reconocer. La B inventa un header: la ventana de Haiku 4.5 es de 200K y no hay beta que la estire. La C confunde capas — `.mcp.json` configura a qué servidores MCP se conecta el agente y no tiene nada que decir sobre ventanas de contexto; `extended_context` no es uno de sus ajustes. La D confunde los dos límites de tokens: `max_tokens` limita lo que el modelo *genera*, mientras que la ventana de contexto limita *entrada más salida*. Subir `max_tokens` por encima de la ventana no la agranda; solo invalida la solicitud.\n\nCuando la entrada de verdad no cabe, las palancas reales son la edición de contexto (limpiar resultados de herramienta ya obsoletos) y la compactación (resumir turnos anteriores), no un header imaginario.\n\nCómo implementarlo:\n```python\nimport anthropic\n\nclient = anthropic.Anthropic()\n\n# La solución: un modelo cuya ventana sea mayor que el problema.\nresponse = client.messages.create(\n    model=\"claude-sonnet-5\",   # 1M de contexto, nativo\n    max_tokens=4096,           # tope de salida, ajeno a la ventana\n    messages=[{\"role\": \"user\", \"content\": hilo_del_ticket}],\n)\n```\n\n```python\n# Si hay que seguir en un modelo de 200K, poda el contexto en vez de inventar un header.\nresponse = client.beta.messages.create(\n    model=\"claude-haiku-4-5\",\n    max_tokens=4096,\n    betas=[\"context-management-2025-06-27\"],\n    context_management={\"edits\": [{\"type\": \"clear_tool_uses_20250919\"}]},\n    messages=[{\"role\": \"user\", \"content\": hilo_del_ticket}],\n)\n```",
    keyConcept: "Un modelo con ventana de 1M gana a un header inventado; max_tokens es el tope de salida, no la ventana",
  },
  ...
  "E1-013": {
    question:
      "Una tubería de integración continua usa Claude Code para revisar pull requests. El prompt de sistema dice que el agente debe ejecutar la batería de pruebas antes de publicar cualquier comentario de revisión, pero aproximadamente una ejecución de cada veinte publica el comentario primero. El equipo necesita que el orden se cumpla siempre. ¿Qué debe implementar?",
    options: {
      A: "Reescribir el prompt de sistema con lenguaje más contundente — «CRÍTICO: DEBES ejecutar las pruebas antes de comentar» — y añadir la regla también a CLAUDE.md",
      B: "Un hook PreToolUse que bloquee la herramienta de publicar comentarios hasta que la de ejecutar pruebas haya terminado con éxito en esta sesión",
      C: "Bajar el nivel de esfuerzo para que el modelo siga la instrucción más al pie de la letra y deje de improvisar",
      D: "Dar al agente una sola herramienta combinada que ejecute las pruebas y publique el comentario en una única llamada",
    },
    explanation:
      "Cuando el cumplimiento tiene que ser determinista, el prompt es la capa equivocada. Un prompt es una instrucción probabilística dirigida al modelo: tiene una tasa de fallo distinta de cero, que es justo ese uno de cada veinte. Un hook PreToolUse es código de la aplicación — inspecciona la llamada pendiente y la rechaza si no se cumple el prerrequisito, así que la tasa de fallo es cero por construcción.\n\nLa opción A sube el tono pero no cambia el mecanismo, y el lenguaje contundente tiende a disparar de más en otros sitios sin cerrar este hueco. La C confunde esfuerzo con obediencia: el esfuerzo ajusta la profundidad del razonamiento, no el seguimiento de reglas, y bajarlo hace que el agente acote más el trabajo, no que respete mejor un orden. La D funcionaría, pero renuncia a poder ejecutar pruebas sin comentar y esconde el fallo: si las pruebas fallan, la herramienta combinada igual tiene que decidir qué publica.\n\nLa regla práctica: el prompt para las preferencias, las puertas programáticas para las garantías.\n\nCómo implementarlo:\n```typescript\nconst estado = { pruebasVerdes: false };\n\nconst hooks = {\n  PreToolUse: async ({ toolName }) => {\n    if (toolName === \"post_review_comment\" && !estado.pruebasVerdes) {\n      return {\n        decision: \"deny\",\n        reason: \"Ejecuta primero las pruebas: post_review_comment depende de una corrida en verde.\",\n      };\n    }\n  },\n  PostToolUse: async ({ toolName, result }) => {\n    if (toolName === \"run_tests\") estado.pruebasVerdes = result.exitCode === 0;\n  },\n};\n```",
    keyConcept: "El prompt para preferencias, los hooks para garantías: un prerrequisito va en código",
  },

  "E1-014": {
    question:
      "Un agente de extracción obtiene registros de tres servidores MCP. Uno devuelve las fechas como marcas de tiempo Unix, otro como cadenas ISO 8601 y el tercero como DD/MM/AAAA. El agente ordena mal los registros porque compara los tres formatos como si fueran el mismo. ¿Cuál es el arreglo más fiable?",
    options: {
      A: "Un hook PostToolUse que normalice cada campo de fecha a ISO 8601 antes de que el resultado de la herramienta llegue al modelo",
      B: "Describir los tres formatos en el prompt de sistema e indicarle al modelo que los convierta antes de comparar",
      C: "Subir el nivel de esfuerzo para que el modelo razone con más cuidado sobre cada fecha",
      D: "Pedir a los responsables de los tres servidores MCP que se pongan de acuerdo en un único formato de fecha",
    },
    explanation:
      "Los hooks PostToolUse interceptan el resultado de una herramienta de vuelta y pueden transformarlo antes de que el modelo lo vea. Normalizar allí los tres formatos hace que el modelo reciba una sola forma coherente y que la comparación deje de ser un problema de razonamiento: pasa a ser un problema de datos, resuelto una vez y en código.\n\nLa opción B mete una conversión mecánica dentro del razonamiento del modelo, donde cuesta tokens en cada registro y aun así falla a veces. La C es la misma apuesta con más presupuesto: razonar más no vuelve fiable un paso que no lo es. La D es la solución más limpia en teoría y la menos disponible en la práctica — rara vez controlas tres servidores ajenos, y aunque los controles, el cambio sale en su calendario de versiones, no en el tuyo. Normaliza en tu frontera.\n\nCómo implementarlo:\n```typescript\nconst aISO = (v: unknown): string | unknown => {\n  if (typeof v === \"number\") return new Date(v * 1000).toISOString();\n  if (typeof v === \"string\") {\n    const m = v.match(/^(\\d{2})\\/(\\d{2})\\/(\\d{4})$/);   // DD/MM/AAAA\n    if (m) return new Date(`${m[3]}-${m[2]}-${m[1]}`).toISOString();\n  }\n  return v;                                              // ya venía en ISO\n};\n\nconst hooks = {\n  PostToolUse: async ({ result }) => ({\n    ...result,\n    records: result.records.map((r) => ({ ...r, date: aISO(r.date) })),\n  }),\n};\n```",
    keyConcept: "PostToolUse normaliza formatos heterogéneos antes de que el modelo los vea",
  },

  "E1-015": {
    question:
      "Ayer una ingeniera usó una sesión con nombre para analizar el módulo de pagos y acordó con el agente un plan de refactorización. Durante la noche un compañero reescribió casi todo ese módulo. Hoy quiere continuar el trabajo. ¿Cuál es la forma más fiable de retomarlo?",
    options: {
      A: "Reanudar la sesión con --resume; el agente notará que los archivos cambiaron y volverá a leerlos",
      B: "Empezar una sesión nueva sembrada con un resumen estructurado de las conclusiones de ayer, porque los resultados de herramienta de la sesión vieja describen código que ya no existe",
      C: "Usar fork_session para ramificar desde la sesión de ayer y trabajar sobre la rama",
      D: "Reanudar la sesión y subir el nivel de esfuerzo para que el agente revise cada archivo antes de actuar",
    },
    explanation:
      "Una sesión reanudada se trae consigo sus resultados de herramienta antiguos, y esos resultados son lecturas de un código que desde entonces se reescribió. El agente razonará sobre ese contenido rancio salvo que algo le obligue a releer, y nada lo hace por su cuenta. Empezar de cero con un resumen estructurado conserva lo que sigue siendo cierto (las conclusiones y el plan) y descarta lo que no (el contenido de los archivos).\n\nLa regla de decisión: reanudar cuando el contexto previo sigue siendo válido en su mayor parte; empezar de cero con un resumen inyectado cuando los resultados de herramienta se han quedado rancios. Aquí el código cambió por debajo, así que esos resultados son justo lo que conviene tirar.\n\nLa opción A da por hecha una relectura que no ocurre sola. La C es la herramienta correcta para otro problema — fork_session ramifica desde una base compartida para explorar caminos divergentes, pero una rama de una sesión rancia sigue siendo rancia. La D paga por razonar más sobre entradas equivocadas.\n\nCómo implementarlo:\n```bash\n# El contexto previo sigue valiendo → reanudar\nclaude --resume analisis-pagos\n\n# Los resultados de herramienta están rancios → sesión nueva, conclusiones a mano\nclaude \"Retomamos el refactor de pagos. Contexto de la sesión anterior:\n- Conclusión: PaymentProcessor mezcla cobro y notificación.\n- Plan acordado: extraer NotificationService y dejar el cobro sin efectos.\n- Aviso: el módulo se reescribió anoche; léelo de nuevo antes de proponer nada.\"\n```",
    keyConcept: "Reanudar arrastra resultados de herramienta viejos; con código cambiado, empezar de cero con un resumen",
  },

  "E1-016": {
    question:
      "Una tubería de extracción usa un coordinador con dos subagentes: uno analiza documentos y otro valida la salida contra un esquema. El equipo propone que el validador lea directamente los resultados del analizador, con el argumento de que pasar por el coordinador es un salto inútil. ¿Cuál es el argumento más fuerte para mantener al coordinador en medio?",
    options: {
      A: "Los subagentes no pueden intercambiar información bajo ninguna circunstancia, así que no hay diseño alternativo",
      B: "Los subagentes no heredan el contexto unos de otros, así que el coordinador tiene que pasar la salida del analizador de forma explícita de todos modos — y enrutar por él concentra observabilidad, manejo de errores y control del flujo de información en un solo sitio",
      C: "El coordinador cachea los resultados de los subagentes, así que un salto directo volvería a ejecutar el analizador en cada validación",
      D: "Los dos subagentes comparten una misma ventana de contexto, así que un intercambio directo contaría los tokens dos veces",
    },
    explanation:
      "El patrón de eje y radios no es burocracia. Cada subagente corre con contexto aislado — no hereda automáticamente el historial de conversación del coordinador ni la salida de ningún hermano—, así que lo que el validador necesite tiene que ponerlo en su prompt quien lo invoque. Eso ya es tarea del coordinador. Y una vez que lo está haciendo, enrutar el resto por él sale gratis y compra un único sitio donde ver qué se pasó, capturar y clasificar los fallos, y decidir qué información puede circular hacia dónde.\n\nLa opción A exagera la restricción: se pueden diseñar traspasos directos, solo que son más difíciles de observar y de recuperar. La C y la D inventan mecánica — el coordinador no es una caché y los subagentes no comparten ventana de contexto; ese aislamiento es justamente el sentido del patrón.\n\nCómo implementarlo:\n```typescript\n// El coordinador pasa explícitamente lo que el validador necesita:\n// el subagente no ve nada de la conversación anterior.\nconst analizado = await Task({\n  subagent_type: \"parser\",\n  prompt: `Extrae los campos de este documento y devuélvelos como JSON.\\n\\n${documento}`,\n});\n\nconst validado = await Task({\n  subagent_type: \"validator\",\n  prompt: `Valida este JSON contra el esquema.\\n\\nJSON:\\n${analizado}\\n\\nEsquema:\\n${esquema}`,\n});\n```",
